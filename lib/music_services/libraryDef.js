...
function searchLibrary(type, term) {
  ...
  logger.info("Search lib for: " + term);
...
function searchAlbums(artist) {
  logger.info("Search album for artist: " + artist);
  const albumsFuzzy = fuzzyAlbums.search(artist);
  const albums = {};

  for (var i = 0; i < albumsFuzzy.length; i++) {
    const a = albumsFuzzy[i];

    if (albums[a.albumName] === undefined || albums[a.albumName] === null) {
        albums[a.albumName] = [];
    }

    albums[a.albumName].push({
        artist: a.artistName,
        album: a.albumName,
        trackNumber: a.albumTrackNumber,
        uri: a.uri,
        metadata: a.metadata
    });
  }
